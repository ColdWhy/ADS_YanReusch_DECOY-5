...
def generate_key():
    return ''.join(secrets.choice(key_pool) for _ in range(15))
...
def separate_text(text_list):
    ...
    for char in text_list:
        block.append(char)
        if len(block) == 5:
            arranged_text.append(''.join(block))
            block = []
...
def decrypt_r(master_key, cipher_table, text):
    ...
    for block in arranged_text:
        ...
        new_block_num = (block_num - position - num_key[i1] - num_key[i2] - num_key[i3]) % len_table
        # in decryption, block_num always comes before the rest
        plaintext_blocks.append(cipher_table[new_block_num])

        position = block_num
        # in decryption, position is block_num and not new_block_num
        index += 1

    plaintext = ''.join(plaintext_blocks)
    return plaintext

def decrypt(sub_key, cipher_table, arranged_text):
    text_blocks = []
    ...
    for block in arranged_text:
        ...
        new_block_num = (block_num - position - num_key[i1] - num_key[i2] - num_key[i3]) % len_table
        # in decryption, block_num always comes before the rest
        text_blocks.append(cipher_table[new_block_num])
